...
// Credenciales del servicio (agrega tus credenciales aquí)
...
async function generateOAuth2Token() {
  try {
    ...
    tokenExpiration = Date.now() + 55 * 60 * 1000; // Actualiza la expiración (55 minutos)
...
// Middleware para verificar y generar el token si es necesario
...
// Endpoint para que el Photon obtenga las programaciones
app.get("/getProgramaciones", ensureValidToken, async (req, res) => {
  try {
    const url =
    ...
    if (!response.ok) {
      throw new Error(
        `Error en la consulta a Firebase: ${response.status} ${response.statusText}`
      );
    }

    const data = await response.json();

    // Extraer solo los dispositivos
