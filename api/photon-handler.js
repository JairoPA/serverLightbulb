...
module.exports = async (req, res) => {
  if (req.method === "GET") {
    // Procesar solicitudes GET para consultar datos
    ...
    // Procesar solicitudes POST para actualizar horarios
    const { userId, pin, horarios } = req.body;

    if (!userId || !pin || !horarios) {
      return res.status(400).json({
        error: "Faltan datos obligatorios: userId, pin o horarios.",
      });
    }
    ...
    try {
      ...
      // Actualiza los horarios en el documento correspondiente
      ...
      return res.status(500).json({ error: "Error interno al actualizar horarios." });
    }
  } else {
